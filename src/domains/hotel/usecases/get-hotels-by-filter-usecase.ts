...
export class GetHotelsByFilterUsecase
  implements IGetHotelsByFilterUsecase
{
  ...
  async execute(
    ...
    const totalHotels = await this.countHotelsByFilterRepository.count(
      filters
    );
    ...
    const hotels = await this.getHotelsByFilterRepository.get(
      restFilterParams,
    );
